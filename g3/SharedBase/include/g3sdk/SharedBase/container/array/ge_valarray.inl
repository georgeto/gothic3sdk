...
void bTValArray<T>::Reserve(GEInt _n)
{
    if (_n > this->GetCapacity())
    {
        ...
        // g_ZeroMemory( this->m_pArray + this->GetCapacity(), sizeof( T ) * static_cast< GEU32 >( _n -
        // this->GetCapacity() ) );
...
template <typename T>
bTValArray<T>::bTValArray(bTValArray<T> const &_s) : bTArrayBase<T>()
{
    this->CopyFrom(_s);
}

template <typename T>
bTValArray<T>::bTValArray(GEInt _n) : bTArrayBase<T>()
{
    this->Reserve(_n);
}

template <typename T>
bTValArray<T>::bTValArray() : bTArrayBase<T>()
{}
