...
#define __BREAK_BLOCK( N )                                                 \
    ...
    if(__CondIncrementCounter( &a_rRunTimeStack.AccessAt( a_rRunTimeStack.GetCount() - 1 ).m_iBreakBlock, GE_PASTE_TOKENS(__breakBlock, N) ))
...
#define __PREPEND_BREAK_BLOCK( N, INCREMENT )                                                        \
    ...
    if(__CondIncrementCounterEquals( &a_rRunTimeStack.AccessAt( a_rRunTimeStack.GetCount() - 1 ).m_iBreakBlock, GE_PASTE_TOKENS(__breakBlock, N), INCREMENT ))
...
#define RUN_AI_HOLD_INVENTORY_ITEMS( LEFT_ITEM, RIGHT_ITEM ) RUN_AI_HOLD_INVENTORY_ITEMS_EXT( SelfEntity, LEFT_ITEM, RIGHT_ITEM )
