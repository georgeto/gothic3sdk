...
template< typename CLASS, typename BASECLASS >
bTPropertyObjectType<CLASS, BASECLASS>::bTPropertyObjectType(void): bCPropertyObjectTypeBase(GETrue), m_PropertyObjectFactory(GetClassName())
{
    bCPropertyObjectSingleton::GetInstance().RegisterTemplate(this);
}

template< typename CLASS, typename BASECLASS >
bTPropertyObjectType<CLASS, BASECLASS>::~bTPropertyObjectType(void)
{
}
...
{
    GE_IMPROPER_USAGE("It is not allowed to make a copy of propertytype.\nDo not call it from derived class or change access !")
    return bCPropertyObjectTypeBase::CopyFrom(a_Source);
}

template< typename CLASS, typename BASECLASS >
bCPropertyObjectTypeBase * bTPropertyObjectType<CLASS, BASECLASS>::DoClone(void) const
{
    GE_IMPROPER_USAGE("It is not allowed to make a clone of propertytype.\nDo not call it from derived class or change access !")
    return 0;
}
