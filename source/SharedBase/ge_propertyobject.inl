...
template< typename CLASS, typename BASECLASS >
bCPropertyObjectBase * bTPropertyObject<CLASS, BASECLASS>::DoClone(void) const
{
...
template< typename CLASS, typename BASECLASS >
void bTPropertyObject<CLASS, BASECLASS>::Create(void)
{
    CreateInternal(GEFalse);
}

template< typename CLASS, typename BASECLASS >
void bTPropertyObject<CLASS, BASECLASS>::Destroy(void)
{
...
GEU16 bTPropertyObject<CLASS, BASECLASS>::ReadData(bCIStream & a_IStream, GEU16 a_u16Version)
{
    ...
    if(!u16PropertyObjectVersion)
        GE_FATAL_ERROR(__FUNCDNAME__" - illegal propertyobject version detected. Can¦t read version ! Filebounds exceeded ?")

    bCAccessorCreator AccessorCreator(this);
    bCAccessorProperty AccessorProperty(AccessorCreator);
    GEBool bFastMode = GETrue; // Assume properties are stored in the same order, as the accessor iterates over them and there are no obsolete properties
    ...
    for(GEU32 i = 0; i < u32PropertyCount; i++)
    {
        ...
        if(pPropertyType && (u16PropertyObjectVersion < 0x1E || pPropertyType->GetPropertyClassName() == strPropertyClassName))
        {
            pPropertyType->PropertyRead(a_IStream, *this);
        }
        else
        {
            static bCCriticalSectionWin32 Mutex; // warning C4640: construction of local static object is not thread-safe (but also done this way in original code)
            Mutex.Acquire();
            {
                static bTPropertyType<CLASS, bCObsoleteClass> DummyProperty(bTPropertyObjectType<CLASS, BASECLASS>::GetInstance(), 0, "", bEPropertyType_PropertyContainer);
                DummyProperty.PropertyDeprecate(a_IStream, *this, strPropertyName, strPropertyClassName, pPropertyType != 0);
            }
            Mutex.Release();
        }
