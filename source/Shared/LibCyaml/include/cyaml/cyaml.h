...
typedef struct cyaml_schema_value {
	...
	cyaml_schema_value(enum cyaml_type _type, enum cyaml_flag _flags, uint32_t _data_size)
	    : type(_type), flags(_flags), data_size(_data_size)
	{
	}

	cyaml_schema_value(enum cyaml_type _type, enum cyaml_flag _flags, uint32_t _data_size, uint32_t _string_min, uint32_t _string_max)
	    : type(_type), flags(_flags), data_size(_data_size)
	{
	    string.min = _string_min;
	    string.max = _string_max;
	}

	cyaml_schema_value(enum cyaml_type _type, enum cyaml_flag _flags, uint32_t _data_size, const struct cyaml_schema_field *_mapping_fields)
	    : type(_type), flags(_flags), data_size(_data_size)
	{
	    mapping.fields = _mapping_fields;
	}

	cyaml_schema_value(enum cyaml_type _type, enum cyaml_flag _flags, uint32_t _data_size, const struct cyaml_schema_value *_sequence_entry, uint32_t _sequence_min, uint32_t _sequence_max)
	    : type(_type), flags(_flags), data_size(_data_size)
	{
	    sequence.entry = _sequence_entry;
	    sequence.min = _sequence_min;
	    sequence.max = _sequence_max;
	}

	cyaml_schema_value(enum cyaml_type _type, enum cyaml_flag _flags, uint32_t _data_size, const cyaml_strval_t *_enumeration_strings, uint32_t _enumeration_count, uint32_t, uint32_t)
	    : type(_type), flags(_flags), data_size(_data_size)
	{
	    enumeration.strings = _enumeration_strings;
	    enumeration.count = _enumeration_count;
	}
#endif
...
#ifdef __cplusplus
	cyaml_schema_field(const char *_key)
	    : key(_key)
	{
	}

	cyaml_schema_field(const char *_key, struct cyaml_schema_value _value, uint32_t _data_offset)
	    : key(_key), value(_value), data_offset(_data_offset)
	{
	}

	cyaml_schema_field(const char *_key, struct cyaml_schema_value _value, uint32_t _data_offset, uint32_t _count_size, uint8_t _count_offset)
	    : key(_key), value(_value), data_offset(_data_offset), count_offset(_count_offset), count_size(_count_size)
	{
	}
#endif
...
#define CYAML_VALUE_INT( \
		_flags, _type) \
	cyaml_schema_value_t(CYAML_INT, (_flags), sizeof(_type))
...
#define CYAML_FIELD_INT( \
	...
	cyaml_schema_field_t(_key, \
		CYAML_VALUE_INT((_flags), ((_structure *)NULL)->_member), \
	offsetof(_structure, _member))
...
#define CYAML_VALUE_UINT( \
		_flags, _type) \
	cyaml_schema_value_t(CYAML_UINT, (cyaml_flag)_flags, sizeof(_type))
...
#define CYAML_VALUE_BOOL( \
		_flags, _type) \
	cyaml_schema_value_t(CYAML_BOOL, (cyaml_flag)_flags, sizeof(_type))
...
#define CYAML_VALUE_ENUM( \
	...
	cyaml_schema_value_t(CYAML_ENUM, (cyaml_flag)_flags, sizeof(_type), _strings, _strings_count, 0, 0)
...
#define CYAML_VALUE_FLAGS( \
		_flags, _type, _strings, _strings_count) \
	cyaml_schema_value_t(CYAML_FLAGS, (cyaml_flag)_flags, sizeof(_type), _strings, _strings_count)
...
#define CYAML_VALUE_FLOAT( \
		_flags, _type) \
	cyaml_schema_value_t(CYAML_FLOAT, (cyaml_flag)_flags, sizeof(_type))
...
#define CYAML_VALUE_STRING( \
	...
	cyaml_schema_value_t(CYAML_STRING, (cyaml_flag)_flags, sizeof(_type), _min, _max)
...
#define CYAML_VALUE_MAPPING( \
	...
	cyaml_schema_value_t(CYAML_MAPPING, (cyaml_flag)_flags, sizeof(_type), _fields)
...
#define CYAML_VALUE_SEQUENCE( \
	...
	cyaml_schema_value_t(CYAML_SEQUENCE, (cyaml_flag)_flags, sizeof(_type), _entry, _min, _max)
...
#define CYAML_FIELD_SEQUENCE( \
	...
	cyaml_schema_field_t(_key, \
		CYAML_VALUE_SEQUENCE((_flags), \
				...
				_entry, _min, _max), \
	offsetof(_structure, _member), \
	sizeof(((_structure *)NULL)->_member ## _count), \
	offsetof(_structure, _member ## _count))
...
#define CYAML_FIELD_SEQUENCE_COUNT( \
	...
	cyaml_schema_field_t(_key, \
		CYAML_VALUE_SEQUENCE((_flags), \
				...
				_entry, _min, _max), \
	offsetof(_structure, _member), \
	sizeof(((_structure *)NULL)->_count), \
	offsetof(_structure, _count))
...
#define CYAML_VALUE_SEQUENCE_FIXED( \
	...
	cyaml_schema_value_t(CYAML_SEQUENCE_FIXED, (cyaml_flag)_flags, sizeof(_type), _entry, _count, _count)
 ...
 */
#define CYAML_FIELD_END cyaml_schema_field_t( NULL )
