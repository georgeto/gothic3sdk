		...
		const cyaml_ctx_t *ctx)
{
	...
	for (idx = ctx->stack_idx - 1; idx != 0; idx--) {
		...
		const uint8_t *data)
{
	...
	if (err == CYAML_OK) {
		...
		for (i = 0; i < schema->enumeration.count; i++) {
		...
		const cyaml_data_t *data)
{
	...
	static /* const */  cyaml_read_scalar_fn fn[CYAML__TYPE_COUNT]; STATIC_INIT // = {
		fn[CYAML_INT]    = cyaml__write_int;
		fn[CYAML_UINT]   = cyaml__write_uint;
		fn[CYAML_BOOL]   = cyaml__write_bool;
		fn[CYAML_ENUM]   = cyaml__write_enum;
		fn[CYAML_FLOAT]  = cyaml__write_float;
		fn[CYAML_STRING] = cyaml__write_string;
	};
		...
		yaml_emitter_t *emitter)
{
	cyaml_ctx_t ctx = { 0 };
	cyaml_err_t err = CYAML_OK;
	typedef cyaml_err_t (* cyaml_write_fn)(
			cyaml_ctx_t *ctx);
	static /* const */ cyaml_write_fn fn[CYAML_STATE__COUNT]; STATIC_INIT // = {
		fn[CYAML_STATE_START]        = cyaml__write_start;
		fn[CYAML_STATE_IN_STREAM]    = cyaml__write_stream;
		fn[CYAML_STATE_IN_DOC]       = cyaml__write_doc;
		fn[CYAML_STATE_IN_MAP_KEY]   = cyaml__write_mapping;
		fn[CYAML_STATE_IN_MAP_VALUE] = cyaml__write_mapping;
		fn[CYAML_STATE_IN_SEQUENCE]  = cyaml__write_sequence;
	};

	// = {
		ctx.config = config;
		ctx.emitter = emitter;
		ctx.seq_count = seq_count;
	// };
		...
		unsigned seq_count)
{
	cyaml_err_t err;
	yaml_emitter_t emitter;
	cyaml_buffer_ctx_t buffer_ctx = { 0 }; // = {
		buffer_ctx.config = config;
		buffer_ctx.err = CYAML_OK;
	// };
