...
#define STATIC_INIT \
	STATIC_INIT_IMPL(__static_init_guard_ ## __COUNTER__)

#define STATIC_INIT_IMPL(_varname) \
	static bool _varname = true; \
	if( _varname ) { \
		_varname = false;
...
{
	static const char * /* const */ strings[CYAML_STATE__COUNT]; STATIC_INIT // = {
		strings[CYAML_STATE_START]        = "start";
		strings[CYAML_STATE_IN_STREAM]    = "in stream";
		strings[CYAML_STATE_IN_DOC]       = "in doc";
		strings[CYAML_STATE_IN_MAP_KEY]   = "in mapping (key)";
		strings[CYAML_STATE_IN_MAP_VALUE] = "in mapping (value)";
		strings[CYAML_STATE_IN_SEQUENCE]  = "in sequence";
	};
