		...
		uint8_t *data)
{
	...
	for (i = 0; i < CYAML_ARRAY_LEN(false_strings); i++) {
		...
		uint8_t *data)
{
	...
	unsigned i;
	for (i = 0; i < CYAML_ARRAY_LEN(fns); i++) {
		...
		yaml_event_t *event)
{
	...
	static /* const */  cyaml_read_scalar_fn fn[CYAML__TYPE_COUNT]; STATIC_INIT // = {
		fn[CYAML_INT]    = cyaml__read_int;
		fn[CYAML_UINT]   = cyaml__read_uint;
		fn[CYAML_BOOL]   = cyaml__read_bool;
		fn[CYAML_ENUM]   = cyaml__read_enum;
		fn[CYAML_FLOAT]  = cyaml__read_float;
		fn[CYAML_STRING] = cyaml__read_string;
	};
		...
		yaml_event_t *event)
{
	...
	typedef cyaml_err_t (* cyaml_read_fn)(
			...
			yaml_event_t *event);
	cyaml_read_fn fn;
	static /* const */ cyaml_read_fn fns[CYAML_STATE__COUNT][CYAML_EVT__COUNT]; STATIC_INIT // = {
		fns[CYAML_STATE_START] // = {
			[CYAML_EVT_STRM_START] = cyaml__stream_start;
		// },
		fns[CYAML_STATE_IN_STREAM] // = {
			[CYAML_EVT_DOC_START]  = cyaml__doc_start;
		fns[CYAML_STATE_IN_STREAM]
			[CYAML_EVT_STRM_END]   = cyaml__stream_end;
		// },
		fns[CYAML_STATE_IN_DOC] // = {
