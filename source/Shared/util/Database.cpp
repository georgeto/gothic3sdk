...
mCDatabase::mCDatabase( mCLoggerBase & a_Logger )
    : m_File(0), m_Logger(a_Logger), m_uLineNumber(0)
{
}

mCDatabase::mCDatabase( GELPCChar a_strFile, mCLoggerBase & a_Logger )
    : m_File(0), m_Logger(a_Logger), m_uLineNumber(0)
{
    Open(a_strFile);
}

mCDatabase::~mCDatabase( void )
{
    Close();
}
...
void mCDatabase::Close( void )
{
    m_File.Close();
}

GEBool mCDatabase::NextLine( void )
{
...
bCString const & mCDatabase::GetLine(void)
{
    return m_strLine;
}
...
GEUInt mCDatabase::GetEntryCount( void ) const
{
...
GEBool mCDatabase::IsTemplate(GEInt a_iIndex, GELPCChar a_strDescription) const
{
    ...
    {
        GetDefaultLogger().LogFormat("%s in %s(%d): %s\n", a_strDescription, m_strFile, m_uLineNumber, strTemplate);
        return GEFalse;
    }
...
{
    m_Logger.LogFormat("%s in %s(%d): %s\n", a_strDescription, m_strFile, m_uLineNumber, a_strValue);
}

void mCDatabase::LogIncorrectLine( void ) const
{
    m_Logger.LogFormat("Fehlerhafte Zeile in %s(%d): %s\n", m_strFile, m_uLineNumber, m_strLine);
}

void mCDatabase::LogIncorrectLine(GELPCChar a_strDescription) const
{
    m_Logger.LogFormat("%s in %s(%d): %s\n", a_strDescription, m_strFile, m_uLineNumber, m_strLine);
}
